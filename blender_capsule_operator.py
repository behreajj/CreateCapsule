...
def mesh_data_to_bmesh(vs, vts, vns, v_indices, vt_indices, vn_indices, use_smooth_shading=True):
    """
    Internal function that creates a BMesh from coordinates, texture
    coordinates, and normals, and the vertex indices which organizes them into
    vertices. Indices lists are assumed to be equal in length.
    """
    ...
    if calc_mid:
        ...
        for m in range(1, verif_rings_p1):
            ...
            for j in range(0, verif_lons):
                ...
                vs[v_cyl_offset] = (
                    cmpl_fac * v_equator_north[0] + fac * v_equator_south[0],
                    cmpl_fac * v_equator_north[1] + fac * v_equator_south[1],
...
class AddCapsule(bpy.types.Operator):
...
def register():
    bpy.utils.register_class(AddCapsule)
    bpy.types.VIEW3D_MT_mesh_add.append(menu_func)


def unregister():
    bpy.utils.unregister_class(AddCapsule)
