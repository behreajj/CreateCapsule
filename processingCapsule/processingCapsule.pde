...
Mesh3 capsule(
  ...
  // To comply with Processing's topsy-turvy world,
  // transpose y and z, then negate z.
  ...
  for (int j = 0; j < verifLons; ++j) {
    ...
    float s_tex = ( j + 0.5 ) * toTexHorizontal;
    vts[j] = new PVector(s_tex, 1.0);
    vts[idxVtSCap + j] = new PVector(s_tex, 0.0);
  ...
  float vtAspectSouth = 1.0 / 3.0;
  if (profile == CapsuleUvProfile.ASPECT) {
    vtAspectSouth = verifRad / (verifDepth + verifRad + verifRad);
  } else if (profile == CapsuleUvProfile.UNIFORM) {
    vtAspectSouth = (float)halfLats / (verifRingsP1 + verifLats);
  }
  float vtAspectNorth = 1.0 - vtAspectSouth;
  ...
  // Default to triangles because textures flicker with quads.
...
PShapeOpenGL toPShape(PGraphics3D rndr, Mesh3 source) {
...
class Mesh3 {
  Index3[][] indices;
  PVector[] coords, texCoords, normals;
