...
function setup () {
    ...
    rndr = new THREE.WebGLRenderer(renderProps);
...
function createCapsule (
    ...
    const verifLats = latitudes < 2 ? 2 :
        latitudes % 2 != 0 ? latitudes + 1 :
            latitudes;
    ...
    const idxVtSEquator = calcMiddle ? idxVtCyl + verifLonsP1 * verifRings : idxVtCyl;
    ...
    for (let j = 0; j < verifLons; ++j) {
        ...
        vs[idxVNEquator + j].set(
            rhoCosTheta,
            halfDepth,
            -rhoSinTheta);
        vs[idxVSEquator + j].set(
            rhoCosTheta,
            -halfDepth,
            -rhoSinTheta);
    ...
    for (let j = 0; j < verifLons; ++j) {
        ...
        updateFace(
            tris[j], vtLayer[j],
            0, j, 0,
            jNextVt, verifLons + j, jNextVt,
            jnp1, verifLons + jNextVt, jnp1,
            vts, vns);

        updateFace(
            tris[south], vtLayer[south],
